 ...
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#define MSGD(colour, mcol, type, fs, _r, mod, fun, ...)\
...
/* INFO messages */
#define INFOMF(...)         MSGD(INFOT, COL, "INFO", stdout, "\n", MOD, FUN, __VA_ARGS__)
#define INFOM(...)          MSGD(INFOT, COL, "INFO", stdout, "\n", MOD, "" , __VA_ARGS__)
#define INFO(...)           MSGD(INFOT, COL, "INFO", stdout, "\n", "" , "" , __VA_ARGS__)

#define INFOMFNR(...)       MSGD(INFOT, COL, "INFO", stdout, "", MOD, FUN, __VA_ARGS__)
#define INFOMNR(...)        MSGD(INFOT, COL, "INFO", stdout, "", MOD, "" , __VA_ARGS__)
#define INFONR(...)         MSGD(INFOT, COL, "INFO", stdout, "", "" , "" , __VA_ARGS__)


/* WARNING messages */
#define WARNMF(...)         MSGD(WARNT, COL, "WARN", stdout, "\n", MOD, FUN, __VA_ARGS__)
#define WARNM(...)          MSGD(WARNT, COL, "WARN", stdout, "\n", MOD, "" , __VA_ARGS__)
#define WARN(...)           MSGD(WARNT, COL, "WARN", stdout, "\n", "" , "" , __VA_ARGS__)

/* ERROR messages */
#define ERRMF(...)          MSGD(ERRT,  COL, "ERR" , stderr, "\n", MOD, FUN, __VA_ARGS__)
#define ERRM(...)           MSGD(ERRT,  COL, "ERR" , stderr, "\n", MOD, "" , __VA_ARGS__)
#define ERR(...)            MSGD(ERRT,  COL, "ERR" , stderr, "\n", "" , "" , __VA_ARGS__)

/* ERROR messages and exit */
#define AERRMF(...)         ({ MSGD(ERRT,  COL, "ERR" , stderr, "\n", MOD, FUN, __VA_ARGS__); exit(-1); })
#define AERRM(...)          ({ MSGD(ERRT,  COL, "ERR" , stderr, "\n", MOD, "" , __VA_ARGS__); exit(-1); })
#define AERR(...)           ({ MSGD(ERRT,  COL, "ERR" , stderr, "\n", "" , "" , __VA_ARGS__); exit(-1); })
